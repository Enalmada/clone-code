...
export async function processFiles(
	...
	// Fetch files using glob pattern
			...
			if (hookStartIndex !== -1 && hookEndIndex > hookStartIndex) {
				...
				if (hookBlock.includes("{") && hookBlock.includes("}")) {
					...
					if (jsonStartIndex !== -1 && jsonEndIndex !== -1) {
						const hookContentJSON = hookBlock
						...
						try {
							...
							if (hookData.toFile) {
								...
								let newFileContent =
									content
										.substring(0, hookStartIndex)
										.trim() + // Trim to remove leading newline
								...
								// Inside the toFile functionality within processFiles
							...
							if (hookData.addType) {
								...
								const typeDeclarationStart = content.indexOf(
									"export type",
									hookEndIndex,
								);
								...
								if (typeMatch && typeDeclarationStart > -1) {
									const typeDeclarationIndex =
										typeDeclarationStart + typeMatch.index!;
							...
							if (hookData.toPlacement) {
								...
								let blockEndIndex = content.indexOf(
									hookEndMarker,
									blockStartIndex,
								); // Find the start of the end hook
									...
									switch (hookData.toPlacement) {
										...
										case "below": {
											const endCommentIndex =
												content.indexOf(hookEndMarker, hookEndIndex) +
												hookEndMarker.length;
											updatedContent =
				...
				console.error(
					`Invalid hook block format in ${hookName} in file ${file}`,
				);
			}
